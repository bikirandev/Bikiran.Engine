...
- **Automates multi-step processes** — chain together HTTP calls, emails, database queries, and custom logic into a single executable flow.
...
| Node Library             | HTTP requests, conditional logic, loops, waits, email, database queries, transforms, and more              |
...
| **ServiceId**      | A UUID (GUID) that uniquely identifies a specific flow run                   |
...
3. A unique ServiceId (UUID) is generated and a FlowRun record is saved (status: pending)
...
// serviceId = "a1b2c3d4-e5f6-7890-abcd-ef1234567890" — use this to track the run
```
