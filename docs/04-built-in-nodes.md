...
| [EmailSendNode](#emailsendnode)         | `EmailSend`     | Send an email                               |
...
Pauses the flow for a set number of milliseconds.
...
### Response Validation with ExpectValue
...
| `PlaceholderResolver` | Func\<FlowContext, Dictionary\<string, string\>\>? | null     | Build placeholders dynamically from context   |

### Body Resolution Priority

The node uses the first available body option:
...
// Using dynamic HTML from context
...
Runs an EF Core query against your application's database and stores the result in context. This node is generic over your `DbContext` type for type-safe queries.

| Property           | Type                                                 | Default                 | Description                              |
| ------------------ | ---------------------------------------------------- | ----------------------- | ---------------------------------------- |
| `Name`             | string                                               | required                | Node name                                |
| `Query`            | Func\<TContext, CancellationToken, Task\<object?\>\> | required                | The EF Core query                        |
...
**Thread safety note:** The shared context dictionary is not thread-safe. Each parallel branch should write to its own unique key. Do not read a key that another branch is writing to at the same time.
